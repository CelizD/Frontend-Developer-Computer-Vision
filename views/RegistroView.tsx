...
/**
 * Componente de Logs de Eventos
 * Permite buscar y filtrar eventos por nivel
 */
const Logs = () => {
  const { eventLog, t } = useAppContext();

  // Estados locales: búsqueda por texto y filtrado por nivel
  const [searchTerm, setSearchTerm] = useState("");
  const [filterLevel, setFilterLevel] = useState<"all" | EventLog['level']>("all");

  /**
   * Función para asignar color según nivel de evento
   */
  ...
  /**
   * Filtrado de logs:
   * 1. Por nivel (alert, warn, info o all)
   * 2. Por búsqueda en el mensaje traducido
   */
